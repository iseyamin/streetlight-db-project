...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Apr 11, 2026 at 10:23 AM
...
-- Procedures
--
DROP PROCEDURE IF EXISTS `GetLightStatusAndFaultDetails`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetLightStatusAndFaultDetails` (IN `p_light_id` INT)   BEGIN
    SELECT
        s.light_id,
        s.status AS light_status,
        z.area_name,
        -- Get the status of the latest fault report if the light is faulty
        CASE
            WHEN s.status = 'Faulty' THEN fr.status
            ELSE 'N/A'
        END AS latest_report_status,
        -- Get the type of the latest fault report if the light is faulty
        CASE
            WHEN s.status = 'Faulty' THEN fr.fault_type
            ELSE 'N/A'
        END AS latest_fault_type
    FROM
        STREETLIGHT s
    JOIN
        ZONE z ON s.zone_id = z.zone_id
    LEFT JOIN (
        -- Subquery to find the details of the LATEST fault report for the input light_id
        SELECT
            report_id,
            light_id,
            status,
        ...
        ORDER BY
            report_date DESC
        LIMIT 1
    ) fr ON s.light_id = fr.light_id
    WHERE
        s.light_id = p_light_id;
END$$
...
(225, 25, 134, 'Bulb Failure', 'Medium', '2025-01-26', 'Resolved'),
...
--
DROP TRIGGER IF EXISTS `update_light_status2`;
DELIMITER $$
CREATE TRIGGER `update_light_status2` AFTER INSERT ON `fault_report` FOR EACH ROW BEGIN
UPDATE streetlight
SET status='Faulty'
...
(522, 422, 'repaired', 200.50, '2026-04-11');
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `update_fault_report_status_resolved`;
DELIMITER $$
CREATE TRIGGER `update_fault_report_status_resolved` AFTER INSERT ON `maintenance_log` FOR EACH ROW BEGIN
UPDATE FAULT_REPORT
SET status='Resolved'
WHERE report_id=
(
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `update_repair_task_on_repair`;
DELIMITER $$
CREATE TRIGGER `update_repair_task_on_repair` AFTER INSERT ON `maintenance_log` FOR EACH ROW BEGIN
    UPDATE REPAIR_TASK
    SET 
        task_status = 'Completed',
        repair_date = NEW.log_date
    WHERE 
        task_id = NEW.task_id;
END
$$
...
CREATE TABLE IF NOT EXISTS `repair_task` (
  ...
  `task_status` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'Assigned',
  PRIMARY KEY (`task_id`),
...
(422, 225, 302, '2026-04-11', NULL, 'Assigned');
...
(134, 15, 23.842000, 90.444000, '2024-10-01', 'Working'),
